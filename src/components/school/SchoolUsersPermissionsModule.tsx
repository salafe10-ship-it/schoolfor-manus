...
import { Check, ChevronDown, Home, KeyRound, Lock, LogOut, Plus, RefreshCw, Search, ShieldCheck, UserCheck, UserMinus, Users, X } from 'lucide-react';
import { authenticatedRequest } from '../../utils/authenticatedRequest';
import { comparePermissionResources, permissionActionLabel, permissionDisplayLabel, permissionModuleLabel } from '../identity/permissionPresentation';
...
export default function SchoolUsersPermissionsModule({ selectedSchool, selectedBranch, triggerNotification, onBackToMainMenu, canManage = false, canAssign = false }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
    ...
    ]).then(async ([requestsResponse, reviewsResponse, conflictsResponse]) => {
      const [requestsPayload, reviewsPayload, conflictsPayload] = await Promise.all([requestsResponse.json().catch(() => ({})), reviewsResponse.json().catch(() => ({})), conflictsResponse.json().catch(() => ({}))]);
      ...
      if (!cancelled) { setAccessRequests(Array.isArray(requestsPayload.requests) ? requestsPayload.requests : []); setAccessReviews(Array.isArray(reviewsPayload.reviews) ? reviewsPayload.reviews : []); setSodConflicts(Array.isArray(conflictsPayload.conflicts) ? conflictsPayload.conflicts : []); }
  ...
  const permissionLabel = (permission: PermissionDescriptor) => permissionDisplayLabel(permission);
  ...
  if (permissionEditing) {
    ...
    return (
      <section data-identity-module="permissions" className="identity-control-surface identity-decision-surface min-h-screen bg-slate-100 p-3 text-slate-900 sm:p-6" dir="rtl">
        <div className="mx-auto max-w-[96rem]">
          <div role="dialog" aria-modal="true" aria-label={`صلاحيات ${permissionEditing.display_name}`} className="overflow-hidden rounded-[2rem] border border-amber-200 bg-white shadow-2xl">
            <header className="identity-hero bg-gradient-to-l from-slate-950 via-slate-900 to-amber-950 p-5 text-white sm:p-7">
                ...
                return <section key={group.resource} className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition ${isExpanded ? 'border-amber-300 ring-1 ring-amber-100' : 'border-slate-200'}`}>
                  <div className="flex flex-col gap-3 border-b border-slate-200 bg-gradient-to-l from-slate-900 to-slate-800 p-4 text-white lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-[10px] font-bold text-slate-200">منح أو سحب التفويض يتم من رأس كل وحدة</span>
                      <button type="button" disabled={!canAssign || allModulePermissions.every((permission) => centrallyDeniedPermissionKeys.has(permission.permissionKey))} onClick={() => setModuleDirectPermissions(group.resource, true)} className="rounded-xl border border-emerald-300/40 bg-emerald-500/20 px-3 py-2 text-[10px] font-black text-emerald-100 disabled:cursor-not-allowed disabled:opacity-40" aria-label={`منح تفويضات وحدة ${group.label}`}>منح تفويضات وحدة</button>
                      <button type="button" disabled={!canAssign || allModulePermissions.every((permission) => !((permissionDrafts[permissionEditing.id] || []).includes(permission.permissionKey)))} onClick={() => setModuleDirectPermissions(group.resource, false)} className="rounded-xl border border-rose-300/40 bg-rose-500/20 px-3 py-2 text-[10px] font-black text-rose-100 disabled:cursor-not-allowed disabled:opacity-40" aria-label={`سحب التفويض المباشر من وحدة ${group.label}`}>سحب التفويض المباشر</button>
                    </div>
                  </div>
  ...
  return (
    <section data-identity-module="users-permissions" className="identity-control-surface min-h-[70vh] bg-slate-50 p-4 sm:p-6 text-slate-900" dir="rtl">
      <div className="mx-auto max-w-7xl space-y-5">
        <div className="identity-hero relative overflow-hidden rounded-3xl border border-amber-500/20 bg-gradient-to-l from-slate-950 via-slate-900 to-amber-950/40 p-6 text-white shadow-2xl">
        ...
        <div className="identity-metric-grid grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div className="identity-metric-card rounded-2xl border border-slate-200 bg-white p-4"><Users className="mb-2 h-5 w-5 text-amber-600" /><div className="text-2xl font-black">{users.length}</div><div className="text-xs font-bold text-slate-500">إجمالي مستخدمي المدرسة</div></div><div className="identity-metric-card rounded-2xl border border-slate-200 bg-white p-4"><UserCheck className="mb-2 h-5 w-5 text-emerald-600" /><div className="text-2xl font-black">{users.filter((user) => user.status === 'active').length}</div><div className="text-xs font-bold text-slate-500">حسابات نشطة</div></div><div className="identity-metric-card rounded-2xl border border-slate-200 bg-white p-4"><Lock className="mb-2 h-5 w-5 text-amber-600" /><div className="text-2xl font-black">{roles.length}</div><div className="text-xs font-bold text-slate-500">قوالب أدوار منشورة مركزيًا</div></div><div className="identity-metric-card rounded-2xl border border-slate-200 bg-white p-4"><ShieldCheck className="mb-2 h-5 w-5 text-amber-600" /><div className="text-2xl font-black">{permissionCatalog.length}</div><div className="text-xs font-bold text-slate-500">قرارات وصول تفصيلية</div></div></div>

        <nav aria-label="شاشات إدارة المستخدمين والصلاحيات" className="identity-view-tabs flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          {([
      ...
      {activeView === 'users' && <div className="identity-panel overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        {activeView === 'governance' && <section className="identity-panel space-y-5 overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="governance-screen-title">
          ...
          {governanceLoading ? <div className="rounded-2xl bg-slate-50 p-10 text-center text-sm font-bold text-slate-500">جارٍ تحميل سجلات الحوكمة...</div> : <>
            <div className="grid gap-3 sm:grid-cols-4"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="text-2xl font-black text-amber-900">{accessReviews.filter((review) => review.status === 'pending').length}</div><div className="text-xs font-bold text-amber-800">مراجعات معلقة</div></div><div className="rounded-2xl border border-sky-200 bg-sky-50 p-4"><div className="text-2xl font-black text-sky-900">{accessRequests.filter((request) => request.status === 'pending').length}</div><div className="text-xs font-bold text-sky-800">طلبات وصول معلقة</div></div><div className="rounded-2xl border border-rose-200 bg-rose-50 p-4"><div className="text-2xl font-black text-rose-900">{sodConflicts.length}</div><div className="text-xs font-bold text-rose-800">تعارضات فصل المهام</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-2xl font-black text-slate-900">{accessReviews.length}</div><div className="text-xs font-bold text-slate-700">إجمالي سجلات المراجعة</div></div></div>
            <div className="overflow-x-auto rounded-2xl border border-sky-200"><table className="w-full min-w-[860px] text-right text-xs"><thead className="bg-sky-950 text-sky-100"><tr><th className="p-3">المستخدم</th><th className="p-3">الصلاحيات المطلوبة</th><th className="p-3">سبب الطلب</th><th className="p-3">الفترة</th><th className="p-3">الحالة</th></tr></thead><tbody className="divide-y divide-sky-100">{accessRequests.map((request) => <tr key={request.id}><td className="p-3 font-black">{request.user_name || 'مستخدم'}</td><td className="p-3 font-mono">{Array.isArray(request.permission_keys) ? request.permission_keys.join('، ') : '—'}</td><td className="max-w-xs p-3">{request.reason}</td><td className="p-3">{request.starts_at ? new Date(request.starts_at).toLocaleDateString('ar') : 'فوري'} — {request.ends_at ? new Date(request.ends_at).toLocaleDateString('ar') : 'مفتوح'}</td><td className="p-3"><span className={`rounded-full px-2 py-1 font-black ${request.status === 'pending' ? 'bg-amber-50 text-amber-800' : request.status === 'approved' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{request.status}</span>{request.approval_status && <div className="mt-1 text-[10px] text-slate-500">موافقة: {request.approval_status}</div>}</td></tr>)}</tbody></table>{accessRequests.length === 0 && <div className="p-10 text-center text-sm font-bold text-slate-500">لا توجد طلبات صلاحيات مسجلة.</div>}</div>
