...
export default function SchoolUsersPermissionsModule({ selectedSchool, selectedBranch, triggerNotification, onBackToMainMenu, canManage = false }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (users.ok) {
        ...
        setPermissionDrafts(Object.fromEntries(nextUsers.map((user: SchoolUser) => [user.id, (user.directPermissions || []).map((permission) => permission.permissionKey)])));
      }
      if (roles.ok) {
  ...
  return (
    <section className="min-h-[70vh] bg-slate-50 p-4 sm:p-6 text-slate-900" dir="rtl">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          {loading ? <div className="p-12 text-center text-sm font-bold text-slate-500">جارٍ تحميل دليل الهوية...</div> : filteredUsers.length === 0 ? <div className="p-12 text-center text-sm font-bold text-slate-500">لا توجد حسابات مطابقة للتصفية الحالية.</div> : <div className="overflow-x-auto"><table className="min-w-[1180px] w-full text-right text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-4">المستخدم</th><th className="p-4">الوظيفة والقسم</th><th className="p-4">الدور المركزي</th><th className="p-4">تفويض دقيق</th><th className="p-4">الحالة</th><th className="p-4">إجراءات آمنة</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredUsers.map((user) => <tr key={user.id} className="align-top hover:bg-slate-50/70"><td className="p-4"><div className="font-black">{user.display_name}</div><div className="mt-1 text-xs text-slate-500">{user.email || 'بريد غير متاح'}</div><div className="mt-1 text-[10px] text-slate-400">{user.branch_name || 'الفرع الرئيسي'}</div></td><td className="p-4"><div>{user.job_title || '—'}</div><div className="mt-1 text-xs text-slate-500">{user.department || '—'}</div></td><td className="p-4"><div className="flex items-center gap-2"><select aria-label={`الدور للمستخدم ${user.display_name}`} disabled={!canManage} value={roleDrafts[user.id] || user.roles?.[0]?.roleKey || ''} onChange={(event) => setRoleDrafts((drafts) => ({ ...drafts, [user.id]: event.target.value }))} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-bold disabled:cursor-not-allowed disabled:opacity-60">{roles.map((role) => <option key={role.roleKey} value={role.roleKey}>{role.name}</option>)}</select>{canManage && <button disabled={saving || !roleDrafts[user.id] || roleDrafts[user.id] === user.roles?.[0]?.roleKey} onClick={() => void mutate(user, 'assign_role', { roleKey: roleDrafts[user.id] })} className="rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-800 disabled:opacity-40">حفظ</button>}</div><div className="mt-2 flex flex-wrap gap-1">{(roles.find((role) => role.roleKey === (roleDrafts[user.id] || user.roles?.[0]?.roleKey))?.permissions || []).slice(0, 4).map((permission) => <span key={permission.permissionKey} className="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] text-slate-500">{permission.permissionKey}</span>)}</div></td><td className="p-4"><div className="flex items-center gap-2"><span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black text-amber-800">{(user.directPermissions || []).length} صلاحية</span>{canManage && <button type="button" onClick={() => openPermissionEditor(user)} className="rounded-lg bg-amber-700 px-2.5 py-1.5 text-[10px] font-black text-white hover:bg-amber-600">إدارة الصلاحيات</button>}</div>{(user.directPermissions || []).length > 0 && <div className="mt-2 flex max-w-[260px] flex-wrap gap-1">{(user.directPermissions || []).slice(0, 3).map((permission) => <span key={permission.permissionKey} className="rounded bg-amber-50 px-1.5 py-0.5 text-[9px] text-amber-800">{permissionLabel({ permissionKey: permission.permissionKey, resource: permission.resource || permission.permissionKey.split('.')[0], action: permission.action || permission.permissionKey.split('.').slice(1).join('.') })}</span>)}{(user.directPermissions || []).length > 3 && <span className="text-[9px] font-bold text-slate-400">+{(user.directPermissions || []).length - 3}</span>}</div>}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${user.status === 'active' ? 'bg-emerald-50 text-emerald-700' : user.status === 'archived' ? 'bg-slate-100 text-slate-500' : 'bg-amber-50 text-amber-700'}`}>{statusLabels[user.status]}</span>{user.force_password_change && <div className="mt-2 text-[10px] font-bold text-amber-700">يتطلب تغيير كلمة المرور</div>}</td><td className="p-4"><div className="flex flex-wrap gap-2">{canManage && <button onClick={() => { setEditing(user); setForm({ name: user.display_name, email: user.email || '', jobTitle: user.job_title || '', department: user.department || '', initialRole: user.roles?.[0]?.roleKey || roles[0]?.roleKey || '', password: '', branchId: user.branch_id || selectedBranch?.id || '' }); }} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-[10px] font-black hover:bg-slate-50">تحرير</button>}{canManage && <button onClick={() => void mutate(user, 'reset_password')} disabled={saving || user.status === 'archived'} className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-[10px] font-black text-amber-700 disabled:opacity-40"><KeyRound className="ml-1 inline h-3 w-3" />ضبط كلمة المرور</button>}{canManage && <button onClick={() => void mutate(user, 'evict_sessions')} disabled={saving || user.status === 'archived'} className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-[10px] font-black text-slate-700 disabled:opacity-40"><LogOut className="ml-1 inline h-3 w-3" />إنهاء الجلسات</button>}{canManage && <button onClick={() => void mutate(user, 'force_password', { forcePasswordChange: !user.force_password_change })} disabled={saving || user.status === 'archived'} className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-[10px] font-black text-amber-800 disabled:opacity-40">{user.force_password_change ? 'إلغاء إلزام التغيير' : 'إلزام تغيير المرور'}</button>}{canManage && (user.status === 'active' ? <button onClick={() => confirmAction('سيتم إيقاف الحساب وإنهاء جلساته الحالية. هل تريد المتابعة؟') && void mutate(user, 'status', { status: 'suspended' })} disabled={saving} className="rounded-lg bg-rose-50 px-2.5 py-1.5 text-[10px] font-black text-rose-700 disabled:opacity-40"><UserMinus className="ml-1 inline h-3 w-3" />إيقاف</button> : user.status !== 'archived' ? <button onClick={() => void mutate(user, 'status', { status: 'active' })} disabled={saving} className="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-black text-emerald-700 disabled:opacity-40">تفعيل</button> : null)}</div></td></tr>)}</tbody></table></div>}
        </div>
