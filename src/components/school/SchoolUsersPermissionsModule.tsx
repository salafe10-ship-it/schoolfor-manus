...
import { Check, KeyRound, Lock, LogOut, Plus, RefreshCw, Search, ShieldCheck, UserCheck, UserMinus, Users, X } from 'lucide-react';
...
export default function SchoolUsersPermissionsModule({ selectedSchool, selectedBranch, triggerNotification, canManage = false }: Props) {
  ...
  return (
    <section className="min-h-[70vh] bg-slate-50 p-4 sm:p-6 text-slate-900" dir="rtl">
      ...
      {(showCreate || editing) && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4"><form onSubmit={editing ? saveEdit : submitCreate} className="w-full max-w-xl space-y-4 rounded-3xl bg-white p-6 shadow-2xl" dir="rtl"><div className="flex items-center justify-between"><div><h2 className="text-lg font-black">{editing ? 'تحرير بيانات المستخدم' : 'إنشاء مستخدم مدرسة'}</h2><p className="mt-1 text-[10px] font-bold text-slate-500">{editing ? 'عدّل البيانات ثم احفظ التعديل بأمان.' : 'أدخل بيانات المستخدم ثم احفظه داخل مدارس الأسرة فقط.'}</p></div><div className="flex items-center gap-2"><button type="button" onClick={openNewUser} className="rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-[10px] font-black text-indigo-700 hover:bg-indigo-100"><Plus className="ml-1 inline h-3.5 w-3.5" /> مستخدم جديد</button><button type="button" aria-label="إغلاق نافذة المستخدم" onClick={() => { setShowCreate(false); setEditing(null); }}><X className="h-5 w-5" /></button></div></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold">الاسم الكامل<input required minLength={2} autoComplete="name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="mt-1 w-full rounded-xl border p-2.5" /></label><label className="text-xs font-bold">البريد الإلكتروني <span className="font-normal text-slate-500">(اختياري)</span><input type="email" autoComplete="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="يمكن تركه فارغاً" className="mt-1 w-full rounded-xl border p-2.5" />{!editing && <span className="mt-1 block text-[10px] font-normal text-slate-500">عند تركه فارغاً سيُنشئ النظام اسم دخول داخلياً آمناً.</span>}</label><label className="text-xs font-bold">المسمى الوظيفي<input value={form.jobTitle} onChange={(event) => setForm({ ...form, jobTitle: event.target.value })} className="mt-1 w-full rounded-xl border p-2.5" /></label><label className="text-xs font-bold">القسم<input value={form.department} onChange={(event) => setForm({ ...form, department: event.target.value })} className="mt-1 w-full rounded-xl border p-2.5" /></label>{jobs.length > 0 && <label className="text-xs font-bold">الوظيفة من دليل شؤون الموظفين<select value={form.jobId} onChange={(event) => setForm((current) => ({ ...current, jobId: event.target.value }))} className="mt-1 w-full rounded-xl border border-indigo-200 bg-indigo-50 p-2.5"><option value="">مسمى يدوي / غير محدد</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.titleAr}{job.departmentName ? ` — ${job.departmentName}` : ''}</option>)}</select><span className="mt-1 block text-[10px] font-normal text-slate-500">يُحفظ معرف الوظيفة مع المستخدم، بينما يحدد الدور الأمني صلاحيات الدخول.</span></label>}{!editing && <><label className="text-xs font-bold">الدور المعتمد<select required disabled={roles.length === 0} value={form.initialRole} onChange={(event) => setForm({ ...form, initialRole: event.target.value })} className="mt-1 w-full rounded-xl border p-2.5 disabled:bg-slate-100">{roles.length === 0 ? <option value="">لا توجد أدوار معتمدة منشورة</option> : roles.map((role) => <option key={role.roleKey} value={role.roleKey}>{role.name}</option>)}</select>{roles.length === 0 && <span className="mt-1 block text-[10px] font-normal text-rose-600">لا يمكن الحفظ حتى تصل قوالب الأدوار من المدرسة الأم.</span>}</label><label className="text-xs font-bold">كلمة مرور اختيارية<input type="password" minLength={8} maxLength={128} autoComplete="new-password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="اتركها للتوليد الآمن" className="mt-1 w-full rounded-xl border p-2.5" /><span className="mt-1 block text-[10px] font-normal text-slate-500">8 رموز على الأقل. تركها فارغًا يولّد كلمة مؤقتة مع إلزام تغييرها.</span></label></>}</div><div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-600">نطاق الحساب: <b>{selectedBranch?.name || 'الفرع الرئيسي'}</b>. لا يمكن للمستخدم الوصول إلى مدرسة أخرى.</div><div className="flex justify-end gap-2"><button type="button" onClick={() => { setShowCreate(false); setEditing(null); }} className="rounded-xl border px-4 py-2 text-xs font-black">إلغاء</button><button type="submit" disabled={saving || (!editing && roles.length === 0)} className="rounded-xl bg-indigo-600 px-5 py-2 text-xs font-black text-white disabled:opacity-50">{saving ? 'جارٍ الحفظ...' : editing ? 'حفظ التعديل' : 'إنشاء المستخدم'}</button></div></form></div>}
