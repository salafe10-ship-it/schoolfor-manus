...
export default function SuperAdminWorkspaceControl({
  ...
  const publishRelease = async (event: React.FormEvent) => {
    ...
    const enabledOrDisabled = Object.keys(features).length ? features : undefined;
    if (!templateId && !enabledOrDisabled) {
    ...
    try {
      const response = await authenticatedRequest('/api/admin/central/releases', {
        ...
        body: JSON.stringify({
          ...
          featureOverrides: enabledOrDisabled,
          channel,
          title: title.trim(),
    ...
    <div id="super-admin-workspace-control" className="space-y-6 text-right" dir="rtl">
      <section className="rounded-3xl border-2 border-indigo-200 bg-gradient-to-l from-slate-950 via-indigo-950 to-slate-900 p-6 text-white shadow-xl">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-emerald-400/20 bg-emerald-950/30 px-4 py-3 text-[11px] font-black text-emerald-200"><LockKeyhole className="w-4 h-4" /> بيانات المدارس معزولة</div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        <form onSubmit={publishRelease} className="xl:col-span-7 rounded-3xl border border-indigo-200 bg-white p-6 shadow-lg space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3"><div className="text-[10px] leading-5 text-slate-500">{scope === 'global' ? 'سيُنشأ إصدار مستقل لكل مدرسة نشطة.' : 'لن تتأثر المدارس غير المحددة.'}</div><button disabled={isPublishing} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-xs font-black text-white shadow hover:bg-indigo-700 disabled:opacity-50"><Rocket className="w-4 h-4" />{isPublishing ? 'جارٍ الاعتماد...' : 'اعتماد ونشر الإصدار'}</button></div>
        </form>

        <div className="xl:col-span-5 space-y-6">
          <form onSubmit={createTemplate} className="rounded-3xl border border-amber-200 bg-amber-50/70 p-5 shadow"><h3 className="text-sm font-black text-amber-950 flex items-center gap-2"><Layers3 className="w-4 h-4" /> إنشاء مدرسة قالب من إعدادات أساسية</h3><p className="mt-1 text-[10px] leading-5 text-amber-900/70">القالب لا يحتوي بيانات طلاب؛ يحفظ الإعدادات والميزات الافتراضية فقط.</p><div className="mt-3 grid gap-2"><input value={templateForm.key} onChange={(event) => setTemplateForm((current) => ({ ...current, key: event.target.value }))} placeholder="template_key مثل: modern-family-default" className="rounded-xl border border-amber-200 bg-white p-2.5 text-xs font-mono" /><input value={templateForm.name} onChange={(event) => setTemplateForm((current) => ({ ...current, name: event.target.value }))} placeholder="اسم القالب" className="rounded-xl border border-amber-200 bg-white p-2.5 text-xs font-bold" /><select value={templateForm.sourceSchoolId} onChange={(event) => setTemplateForm((current) => ({ ...current, sourceSchoolId: event.target.value }))} className="rounded-xl border border-amber-200 bg-white p-2.5 text-xs font-bold"><option value="">بدون مدرسة مصدر</option>{schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</select><button disabled={isCreatingTemplate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-700 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><ClipboardCopy className="w-4 h-4" />{isCreatingTemplate ? 'جارٍ الحفظ...' : 'حفظ القالب كمسودة'}</button></div></form>
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow"><div className="flex items-center justify-between"><h3 className="text-sm font-black text-slate-900 flex items-center gap-2"><GitBranch className="w-4 h-4 text-indigo-600" />حالة المدرسة المحددة</h3>{selectedWorkspace && <span className="rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-700">إصدار {selectedWorkspace.currentReleaseVersion || 0}</span>}</div>{selectedSchool ? <div className="mt-3 space-y-2 text-[11px] font-bold text-slate-600"><div className="flex justify-between"><span>المدرسة</span><span className="text-slate-900">{selectedSchool.name}</span></div><div className="flex justify-between"><span>القناة</span><span className="text-indigo-700">{selectedWorkspace?.releaseChannel || 'stable'}</span></div><div className="flex justify-between"><span>القالب</span><span className="text-slate-900">{selectedWorkspace?.templateKey || 'الأساس المحلي'}</span></div><button type="button" onClick={() => void publishTemplate()} disabled={!templateId || templates.find((template) => template.id === templateId)?.status === 'published'} className="mt-2 w-full rounded-xl border border-indigo-200 bg-white px-3 py-2 text-[11px] font-black text-indigo-700 disabled:opacity-40">نشر القالب المحدد</button></div> : <div className="mt-3 text-xs text-slate-500">اختر مدرسة لعرض إصدارها.</div>}</div>
        </div>
      </div>
