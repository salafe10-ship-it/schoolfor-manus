...
describe('Student Affairs registration form contract', () => {
  it('exposes the required canonical registration status field', () => {
    ...
    expect(source).toContain('<option value="applicant">طلب تسجيل</option>');
    expect(source).toContain('<option value="admitted">مقبول</option>');
    ...
    expect(source).toContain('<option value="withdrawn">منسحب / منقول</option>');
  });
