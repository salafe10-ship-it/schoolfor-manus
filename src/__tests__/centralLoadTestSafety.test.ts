...
describe('5000-school load test safety contract', () => {
it('defaults to a non-mutating plan and caps the certification baseline at 5000 schools', () => {
    expect(source).toContain("process.env.EDUPRO_LOADTEST_MODE || 'plan'");
    expect(source).toContain('MAX_CERTIFICATION_SCHOOLS = 5000');
  ...
  it('requires an explicit apply confirmation before remote writes', () => {
    ...
    expect(source).toContain('SCHOOLFORMANUS_5000_LOADTEST_APPLY');
