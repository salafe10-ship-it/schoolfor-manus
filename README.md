...
The developer/owner account may be linked directly to one canonical owner
school. Set `OWNER_SCHOOL_ID`, `OWNER_AUTH_USER_ID`, and
`OWNER_SCHOOL_BIND_CONFIRMATION=BIND_OWNER_SCHOOL`, then run
`npm run db:bind-owner-school`. The command is non-destructive, verifies that
the Auth user has the active `platformadmin` assignment and the same school
scope, and marks that school as `owner_controlled`. Global customer releases
exclude this owner school; targeted releases remain explicit and audited.
