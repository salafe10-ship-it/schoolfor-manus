...
async function main() {
  ...
  try {
    ...
    try {
      ...
      `, [tenantId]);
      await client.query(`
        INSERT INTO public.schools (id, tenant_id, school_code, legal_name, display_name, timezone, locale, status, central_metadata)
        VALUES ($1, $2, 'MODERN-FAMILY', $3, $3, 'Africa/Khartoum', 'ar', 'active', $4::jsonb)
      `, [schoolId, tenantId, SCHOOL_NAME, JSON.stringify({
        portal_profile: 'owner_controlled',
        ownerWorkspace: { mode: 'owner', kind: 'owner', releaseChannel: 'stable' },
        features: {},
      })]);
      await client.query(`
