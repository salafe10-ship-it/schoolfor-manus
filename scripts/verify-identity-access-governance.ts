...
try {
  ...
  try {
    const result = await client.query<{
      ...
      reviews_table: boolean;
      sod_rules_table: boolean;
      ...
      grants_rls: boolean;
      reviews_rls: boolean;
      reviews_force_rls: boolean;
      sod_rules_rls: boolean;
      sod_rules_force_rls: boolean;
      ...
      SELECT
        ...
        COALESCE((SELECT c.relrowsecurity FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='identity_access_reviews'), false) AS reviews_rls,
    ...
    const ok = verification.requests_table && verification.approvals_table && verification.grants_table
      && verification.reviews_table && verification.sod_rules_table
