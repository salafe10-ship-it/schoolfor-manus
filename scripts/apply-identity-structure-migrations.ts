...
try {
  ...
  try {
    ...
    const verification = await client.query<{ job_id: boolean; actor_guard: boolean; fee_policy: boolean }>(`
      SELECT
    ...
    `);
    if (!verification.rows[0]?.job_id || !verification.rows[0]?.actor_guard || !verification.rows[0]?.fee_policy) {
